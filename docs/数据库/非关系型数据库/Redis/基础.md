...
Redis没有类似MySQL中的Table的概念，可以通过给key添加前缀加以区分不同类型的key，不过这个前缀不是随便加的，有一定的规范：
...
并且，在Redis的桌面客户端中，会以相同前缀作为层级结构，让数据看起来层次分明，关系清晰
...
- BLPOP和BRPOP：与LPOP和RPOP类似，只不过在没有元素时等待指定时间（单位是秒），而不是直接返回nil
...
是Geolocation的简写，代表地理坐标。Redis中允许存储地理坐标信息，帮助我们根据经纬度来检索数据
...
- GEOSEARCH：在指定范围内搜索member，并按照与指定点之间的距离排序后返回（6.2新功能）
- GEOSEARCHSTORE：与GEOSEARCH功能一致，可以把结果储存到一个指定的key（6.2新功能）
