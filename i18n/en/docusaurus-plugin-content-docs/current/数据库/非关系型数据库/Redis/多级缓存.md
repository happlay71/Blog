...
| table | Lua中表（table）其实是一个"关联数组"，数组的索引可以是数字、字符串或表类型。在Lua里，table的创建是通过"构造表达式"来完成，最简单的构造表达式是{}，用来创建一个空表。|
...
| 操作符 | 描述 |
| :--: | :--: |
| and | 逻辑与。若A为false，则返回A，否则返回B |
| or | 逻辑或。若A为true，则返回A，否则返回B |
| not | 逻辑非。于逻辑运算结果相反 |
...
页面发送请求`http//ip:端口/api/……`就被windows上安装的Nginx服务给接收到了。然后代理给了OpenResty集群：

openresty的nginx.conf文件里的接收代码
```
localhost /api {
...
![](https://happlay-docs.oss-cn-beijing.aliyuncs.com/docs/20240716120706.png)
...
![](https://happlay-docs.oss-cn-beijing.aliyuncs.com/docs/20240716145557.png)
...
OpenResty为Nginx提供了**shard dict**的功能，可以在nginx的多个worker之间共享数据（多个OpenResty），实现缓存功能。
...
```yaml
canal:
  destination: hh # canal的集群名字，要与安装canal时设置的名称一致
...
`Id`: 标记主键
`Column`：属性名与表中字段不一致时使用
`Transient`：该注解标记不属于表中的字段
