  ...
  return (
    <header className={clsx(styles.heroBanner)}>
      <div className={styles.heroContainer}>
        {/* ── 头像 Canvas（点击会触发彩蛋 02） ── */}
        <canvas
          width="280"
          height="280"
          className={styles.heroAvatar}
          onClick={() => _avatarClickHandler?.()}
...
export default function Home() {
  ...
  return (
    <Layout
      ...
      {/* ── 彩蛋组件（挂载到全局，注册头像点击回调） ── */}
      <EasterEgg
        onRegisterAvatarClick={(fn) => {
          _avatarClickHandler = fn;
        }}
      />
...
/* ─── WebGL 工具类（原样保留） ─── */
class WebGL {
  gl; program; vs; fs; buffer; textures = [];
  ...
  setAttrib(name, size, type, normalized, stride, offset) {
    ...
    this.gl.vertexAttribPointer(location1, size, type, normalized, stride, offset);
  ...
  then(callback) { callback.call(this); return this; }
}
